            ...
            >
              ...
              <v-img :src="deal.image" class="mb-4 productImg"></v-img>
                ...
                <div class="d-flex">
                  ...
                  <v-chip class="store-chip my-md-1" small>{{ deal.source }}</v-chip>
                </div>
              </v-card>
            </v-col>
          </v-row>
...
//         const mappedDeals=this.deals.map(item => {
//   if (item.name) {
//     return item.name;
//   }
//   return null;
...
//     },
  ...
  methods: {
      ...
      try {
       if (searchText) {
         res = await this.$http.post(
          ...
          { search:searchText, page: 1, platform: "", is_deals: 1, is_ads: 0, is_web: true }
        );
       } else {
         res = await this.$http.post(
          "https://lendnode.creditclan.com/gateway/store/stores/get_deals",
          { search: "", page: 1, platform: "", is_deals: 1, is_ads: 0, is_web: true }
        );
       }
    ...
    if(this.searchText){
      res = await this.$http.post(
        "https://lendnode.creditclan.com/gateway/store/stores/get_deals",
        { search: this.searchText, page: this.next, platform: "", is_deals: 1, is_ads: 0, is_web: true }
      );
    }else{
      res = await this.$http.post(
        "https://lendnode.creditclan.com/gateway/store/stores/get_deals",
        { search: '', page: this.next, platform: "", is_deals: 1, is_ads: 0, is_web: true }
      );
    }
