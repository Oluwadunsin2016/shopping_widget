    ...
    <div class="d-flex flex-column">
      <div>
        ...
        <v-row v-if="deals.length>0">
          ...
          >
              <!-- :to="{ name: 'deal', params: { id: deal.id,referralCode } }" -->
            <v-card
              flat
              outlined
 ...
 this.setReferralCode();
  ...
  methods: {
    async getItems(searchText) {
      ...
      if (searchText) {
        res = await this.$http.post(
          ...
          { search: searchText, page: 1, is_deals: 1, }
        );
      } else {
        res = await this.$http.post(
          "https://lendnode.creditclan.com/gateway/store/stores/get_nigeria_deals/nigeria",
    ...
    if (this.searchText) {
      ...
      res = await this.$http.post(
        "https://lendnode.creditclan.com/gateway/store/stores/get_nigeria_deals/nigeria",
        { search: this.searchText, page: this.next, is_deals: 1}
      );
    } else {
      res = await this.$http.post(
        "https://lendnode.creditclan.com/gateway/store/stores/get_nigeria_deals/nigeria",
        { search: "", page: this.next, is_deals: 1}
      );
    }
