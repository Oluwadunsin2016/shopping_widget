<template>
  <v-sheet>
    <form class="py-10 px-7">
      ...
      <div
        v-if="item.title === 'Renew Rent'"
        class="text-h4 font-weight-bold title black--text my-5"
      >
          ...
          <v-col cols="12" sm="6" md="6">
            <v-radio-group v-model="houseType">
              ...
              <v-radio
                ...
                value="bungalow"
              ></v-radio>
              <v-radio label="Duplex" color="primary" value="duplex"></v-radio>
            </v-radio-group>
          </v-col>
...
export default {
  ...
  methods: {
    updateHT() {
        this.$store.commit('rent/set', { houseType: this.houseType });
        this.$emit('done')
      }
  },
};
