<template>
  <div>
    <!--  Main Nav -->
    <v-app-bar
      :class="{ scrolled: scrolled }" class="nav-background-on-scroll" color="transparent" elevation="0" fixed
    ...
    >
      <div class="container d-flex align-center position-relative">
        ...
        <v-badge :content="`${$store.state.cart.items.length}`" color="red" overlap>
          <v-btn @click="$store.dispatch('cart/show')" color="black" rounded text>
